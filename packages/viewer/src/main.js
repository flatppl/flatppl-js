...
function rebuildDerivations(ctx) {
...
function buildPlotPlan(ctx, binding /*, bindingsMap */) {
...
function renderConstantRecord(ctx, measure, bindingName) {
  renderTextValue(ctx, bindingName, formatConstantMeasure(ctx, measure));
}

function renderRecordMarginals(ctx, measure, bindingName, extraToolbarControls) {
...
function renderSampleStats(ctx, measure) {
...
function renderFixedRecord(ctx, plan) {
...
function renderKernelSampleForCurrent(ctx) {
...
function buildPresetControl(ctx, plan, onChange) {
...
function buildDomainControl(ctx, plan, onChange) {
  var frag = document.createDocumentFragment();
  if (!plan.axes || plan.axes.length === 0) return frag;
...
function materialiseConcreteMeasure(ctx, ir, count, seed) {
...
function renderProfilePlotForCurrent(ctx) {
  var plan = ctx.currentPlotPlan;
...
function renderProfileLine(ctx, values, range, plan, sweepAxis) {
  var fg = getComputedStyle(document.body).color || '#ccc';
...
function commitSliceX(ctx, plan, x) {
...
function renderArrayStepPlot(ctx, arr) {
  var fg = getComputedStyle(document.body).color || '#ccc';
  var n = arr.length;
...
function renderSamplesAndDensity(ctx, reply, plan) {
...
function updatePlotForBinding(ctx, bindingName) {
...
function drawReificationLassos(ctx, data) {
...
function applySourceUpdate(ctx, msg) {
...
function resizeAllEchartsInPlot(ctx) {
