...
export async function runMcmcPool(ctx: Ctx, name: string, opts: any): Promise<any> {
  ...
  // nChains chains across workers (each runs its own warmup+draws), then recompute
  // split-R̂ / bulk-ESS ONCE over all chains globally (a worker holding a subset
  // can't pool them itself). No freeze phase.
