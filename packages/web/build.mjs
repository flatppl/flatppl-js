...
// Single source of truth for file-type / tree-bucket decisions, shared with
...
console.log('  verified + copied flatppl-theme v0.1.3 -> dist/theme/');
...
const SRC_BUNDLE_ENTRIES = new Set([
  ...
  // Bundled to vendor/file-types.js (IIFE global) below, not served as-is.
  'file-types.mjs',
]);
...
// Shared file-type / tree-bucket module, published as window.FlatPPLFileTypes
// for the runtime scripts (surfaces.ts, app.ts). Same logic build.mjs imports
// directly above — one source, no drift. Loaded as a plain <script> before the
// web sub-modules in index.html.
const fileTypesBuildOpts = {
  entryPoints: [join(here, 'src', 'file-types.mjs')],
  outfile: join(vendorDir, 'file-types.js'),
  bundle: true,
  minify: true,
  format: 'iife',
  globalName: 'FlatPPLFileTypes',
  platform: 'browser',
  target: ['es2020'],
  legalComments: 'inline',
};
...
const BUNDLES = [
  { opts: fileTypesBuildOpts,     label: 'file-types         -> dist/vendor/file-types.js' },
...
//   hideTestExamples — hide the gallery's "Test cases" folder (demo/ +
//                      test-cases/). On when FLATPPL_HIDE_TEST_EXAMPLES is a
...
async function generateManifest() {
  ...
  for (const path of demoFiles) {
    entries.push({ path: 'demo/' + path,     title: basenameTitle(path), type: typeForPath(path) });
  }
  for (const path of exampleFiles) {
    entries.push({ path: 'examples/' + path, title: basenameTitle(path), type: typeForPath(path) });
  }
