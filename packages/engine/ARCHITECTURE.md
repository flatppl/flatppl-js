...
| `broadcast-shape.ts` (~330) | VALUE-level broadcast shape: `classifyNestedJSArray` (outer-rank of a raw nested array, engine-concepts §2.1) + the outer-axis slot descriptors the value-broadcast loop consumes. `tryStackBroadcastCells` stacks per-cell results into one Value, `densify`-ing a structured cell first (value.ts's stated consumer contract). Distinct from `kernel-broadcast-shape.ts` (measure/kernel shapes). |
| `collection-domain-heads.ts` (~215) | The 53 §07 heads across six §07 tables whose Domains cell admits only collections, each with its table and its cell as §07 writes it, plus §04's ten `aggregate`-eligible reductions. Spec §04 hands a broadcast head one ELEMENT and §03 defines no rank-0 array, so a scalar cell under one of these is a static error. TWO consumers, each keeping its own half: `typeinfer.inferBroadcast` (the located diagnostic) and `dissolver` (refuses to discard the wrapper even when inference is bypassed). Carve-outs `min`/`max` and `cat` admit a scalar and are deliberately absent; the head set mirrors `flatppl-rust`'s `COLLECTION_DOMAIN_HEADS`. |
...
SLICE: a `fixed-rank` declaration whose every arg is exactly one rank above its
`argRanks` entry, so `ops.dispatch` atom-batches and slices. That holds for
`cross`/`self_outer`/`diagmat`/`det`/… over a nested operand and fails for every
undeclared head (`cumsum`, `sum`, `diag`, `conv` — legacy whole-value
`ARITH_OPS`) and for the `rank-polymorphic` `transpose`/`adjoint`. It is the
dissolver's own half of the domain rule in `typeinfer.inferBroadcast`, so no
wrapper is discarded when inference is bypassed or `deferred`. A
collection-domain head DEEPER in a compound body always refuses. Fusion (a)
likewise refuses a broadcast-over arg whose cell is itself an array, since
`_wrapVectorLeaves` would treat the emitted `get(arg, .atom)` as a finished
scalar leaf.
