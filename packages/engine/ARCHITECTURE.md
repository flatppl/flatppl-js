...
| `normalize-mass.ts` (~55) | `totalMassExpr(measureIR)` — symbolic IR for Z(θ) of a `superpose`-of-`weighted` (spec §06 normalize/superpose); shared by `mcmc-density` + `mat-density` so a latent-dependent `normalize` scores per-θ; null when not closed-form. |
