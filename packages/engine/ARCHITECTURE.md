...
| `traceeval.ts` (~400) | Per-draw **sample-side** walker for measure IR — the `rand(state, M)` / `builtin_sample` path (`MEASURE_OP_WALKERS` table). No scoring: the old `tally`/score protocol was lifted into `density.ts` long ago. **Being retired** (engine-concepts §11, TODO): the batched materialiser is the single sample evaluator — `iid` is just a batch axis, leaves are `sampleN`; `rand`/`iid` of a forward measure route to a child ctx (fresh cache + split-seed), and shared-vs-fresh draws are a cache-scope choice (session vs child). |
