...
| `linear-gaussian.ts` (~330) | Analytic marginal of a shared-ancestor joint law (spec §06 "Reified components share their ancestry" / "Density of composed measures"). `recogniseGaussianMarginal(body, marginalize, ctx)` proves the shared sub-DAG linear-Gaussian — every node a `Normal` with an AFFINE location in its ancestors and a constant scale, both the keyword and positional spelling — and returns the propagated `{labels, mean, cov}`; anything else returns `{refuse: <reason>}`. `scoreGaussianMarginal(g, observed)` scores it through the canonical `density-prims` MvNormal/Normal closed form (no second implementation of the same maths). `clm.lowerMeasure` calls it for the H8 shared-ancestor reduce and `mat-density.matScore` answers `analytic-gaussian` / throws `refuse` — §06 permits a closed form or a discrete enumeration and "otherwise reports a static error", so there is NO MC branch here. Oracles: Distributions.jl, pinned in `test/joint-shared-ancestor-density.test.ts`. |
