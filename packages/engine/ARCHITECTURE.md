...
| `normalize-mass.ts` | `totalMassExpr(measureIR)` builds symbolic mass expressions for supported measure algebra. Callable weights require integration. `assertFixedMassFallback` refuses unresolved parameter-dependent density normalizers after supported mass builders decline. Both density paths share this scope-aware guard. Fixed scalar leaf normalizers use the same deterministic quadrature in `mat-density` and `mcmc-density`; logweights use a finite scale before exponentiation. |
