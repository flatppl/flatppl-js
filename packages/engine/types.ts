...
const SIGNATURE_FACTORIES = {
  ...
  ChiSquared:        () => realDistKwargs({ k: REAL }),
  VonMises:          () => realDistKwargs({ mu: REAL, kappa: REAL }),
  Laplace:           () => realDistKwargs({ location: REAL, scale: REAL }),
  // Scalar discrete.
  Bernoulli:         () => boolDistKwargs({ p: REAL }),
  Binomial:          () => intDistKwargs({ n: INTEGER, p: REAL }),
  Poisson:           () => intDistKwargs({ rate: REAL }),
  Geometric:         () => intDistKwargs({ p: REAL }),
  NegativeBinomial:  () => intDistKwargs({ alpha: REAL, beta: REAL }),
  NegativeBinomial2: () => intDistKwargs({ mu: REAL, psi: REAL }),
  ...
  // previous `arith2()` signature typed it `(real, real) → real`, which
  // disagreed with the spec's integer domain.
  ...
  asin:   () => ({ args: [REAL], kwargs: {}, result: REAL }),
  acos:   () => ({ args: [REAL], kwargs: {}, result: REAL }),
  atan:   () => ({ args: [REAL], kwargs: {}, result: REAL }),
  atan2:  () => ({ args: [REAL, REAL], kwargs: {}, result: REAL }),
  sinh:   () => ({ args: [REAL], kwargs: {}, result: REAL }),
  cosh:   () => ({ args: [REAL], kwargs: {}, result: REAL }),
  tanh:   () => ({ args: [REAL], kwargs: {}, result: REAL }),
  asinh:  () => ({ args: [REAL], kwargs: {}, result: REAL }),
  acosh:  () => ({ args: [REAL], kwargs: {}, result: REAL }),
  atanh:  () => ({ args: [REAL], kwargs: {}, result: REAL }),
  log1p:  () => ({ args: [REAL], kwargs: {}, result: REAL }),
  expm1:  () => ({ args: [REAL], kwargs: {}, result: REAL }),
