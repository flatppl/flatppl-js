...
//   x ~ Normal(1.1, 0.2)            (marginalised generator draw)
...
test('deriveMcLikelihoodRecipe: auto-derives transport recipe from model bindings, matches oracle', async () => {
  ...
  const src = fs.readFileSync(
    'test/fixtures/simple-transport.flatppl', 'utf8');
  ...
  assert.strictEqual(r.marginalRef.name, 'x', 'marginalised latent should be the generator draw x');
  assert.deepStrictEqual(r.retainedInterval, [0, 1], 'retained Uniform support');
  assert.strictEqual(r.marginalDistIR.op, 'Normal', 'marginal dist is the generator Normal');
